<x-visitor-layout>
    ...
    <div class="text-center mb-1">
        ...
        <div class="flex justify-content-center align-content-center mt-2">
            ...
            <!-- Logo Medik -->
            <svg xmlns="http://www.w3.org/2000/svg" class="ml-1" viewBox="0 0 716 235" width="100" height="50" style="shape-rendering:geometricPrecision;text-rendering:geometricPrecision;image-rendering:optimizeQuality;fill-rule:evenodd;clip-rule:evenodd">
                <defs>
                    <linearGradient id="gradientFill" gradientTransform="rotate(30 .5 .5)">
                        <stop offset="0%" stop-color="#2ad7e5"/>
                        <stop offset="100%" stop-color="#1eb594"/>
                    </linearGradient>
                </defs>
                <path fill="#6c757d" d="M601 115c10 0 14 6 13 13-1 9-8 9-17 9-8-1-10-7-9-14 2-6 5-8 13-8Z
    ...
    <form class="w-full sm:max-w-md mt-2 px-6 py-4 bg-white shadow-md overflow-hidden sm:rounded-lg" method="POST" action="{{ route('login') }}">
        @csrf

        <!-- Email Address -->
        <div class="mb-4">
            <label for="email" class="block font-medium text-sm text-gray-600 ">Correo Electrónico</label>
            <input id="email" placeholder="email@example.com" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full" type="email" name="email" value="{{ old('email') }}" required autofocus autocomplete="username">
            @error('email')
